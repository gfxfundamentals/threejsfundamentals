...
function postHTML(html) {
  ...
  [...document.querySelectorAll('code')].filter(elem => {
    return getKeywordLink(elem.textContent) &&
           elem.parentElement.nodeName !== 'A';
  }).forEach(elem => {
    const a = document.createElement('a');
    a.href = getKeywordLink(elem.textContent);
  ...
  [...document.querySelectorAll('a')].forEach(elem => {
    ...
    if (m) {
      ...
      if (codeKeywordLink) {
        elem.setAttribute('href', `${codeKeywordLink}#${m[2]}`);
      }
    } else if (classRE.test(href)) {
      const codeKeywordLink = getKeywordLink(href);
      if (codeKeywordLink) {
        elem.setAttribute('href', codeKeywordLink);
      }
    }
  });
