Title: Оптимизация большого количества объектов
...
Давайте попробуем это в 3D. Начиная с кода от [рендеринга по требованию](rendering-on-demand.html) мы сделаем один блок на данные в файле. 
...
Обратите внимание на вызов для рендеринга после завершения загрузки текстуры. Нам это нужно, потому что мы выполняем  [рендеринг по требованию](rendering-on-demand.html) 
...
THREE.Object3D, как мы покрывали в [графах сцены](scenegraph.html) 
...
После того, как данные закончили загрузку и анализ, нам нужно выполнить рендеринг хотя бы один раз, поскольку мы выполняем 
 [рендеринг по требованию](rendering-on-demand.html).
...
Мы можем проверить частоту кадров, [открыв devtools](debugging-javascript.html) и включив индикатор частоты кадров браузера.
...
[другой статье](optimize-lots-of-objects-animated.html).
